...
func New(telemetry  ct.Telemetry, service controller.Eventer) *handler {
	return &handler{
		telemetry, service,
	}
}
...
func validateRequest(ctx ct.Context, req *pb.EmitRequest) l.Error {
	ctx, span := ctx.Span()
	defer span.End()
