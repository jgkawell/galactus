...
type Eventer interface {
	PublishAndSave(ctx ct.ExecutionContext, event *eventspb.CloudEvent) (string, l.Error)
}
...
func (e *eventer) PublishAndSave(ctx ct.ExecutionContext, event *eventspb.CloudEvent) (string, l.Error) {
...
func (e *eventer) publish(ctx ct.ExecutionContext, event *eventspb.CloudEvent) l.Error {
	var err error
	success := false
	for i := 0; i < 5; i++ {
		err = e.bus.Publish(ctx.GetContext(), messagebus.PublishParams{
			Event: event,
			// Tags: TODO,
		})
		if err == nil {
...
func (e *eventer) save(ctx ct.ExecutionContext, event *eventspb.CloudEvent, received, published time.Time) {
...
func (e *eventer) failSave(ctx ct.ExecutionContext, event *eventspb.CloudEvent, received, published time.Time, originalErr error) {
	...
	newErr = e.bus.Publish(ctx.GetContext(), messagebus.PublishParams{
		Event: newEvent,
		// Tags: TODO,
	})
	if newErr != nil {
